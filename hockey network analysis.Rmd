---
title: "Anna Cole Network Final: Hockey Network Analysis"
...
* For stacked barplot: https://r-graph-gallery.com/48-grouped-barplot-with-ggplot2.html

* To rotate axis labels: https://www.datanovia.com/en/blog/ggplot-axis-ticks-set-and-rotate-text-labels/

* Match function in betweenness graph: https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/match 
...
setwd("~/Desktop/admin/portfolio/network analysis/final/hockey-network-analysis")
```
...
my_packages <- c('ggplot2', 'igraph', 'RColorBrewer')
```
...
#TAMPA BAY FORMATTING

tbdata <- hockeydata[hockeydata$team == "lightning", ]
...
player_points <- data.frame(
  unique_players = unique(c(tbdata$player_1, tbdata$player_2)),
  assists = rep(0, length(unique(c(tbdata$player_1, tbdata$player_2)))),
  goals = rep(0, length(unique(c(tbdata$player_1, tbdata$player_2))))
)
...
for (i in 1:nrow(tbdata)) {
  ...
  passer <- tbdata$player_1[i]
  receiver <- tbdata$player_2[i]
...
```{r, include = FALSE, message=F, warning=F}
#TAMPA BAY: BARPLOT REFORMATTING
...
ggplot(barplot_data, aes(fill=point_type, y=value, x=players)) + 
  ...
  # set title
  ggtitle("Tampa Bay Lightning: Total Points Earned per Player (Goals and Assists)") +
  
  #rotate axis labels and center them a bit better
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5)) +

  #change the colors
  scale_fill_manual(values = c("assist" = "#898bb2", "goal" = "#002868")) +
  
  #change the y-axis scale to fit the avs
